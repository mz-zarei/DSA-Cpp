...
/**
 * @brief Generic Tree Project
 * @file GenericTreeExercises.cpp
...
/*******************************************************************
  Task 1: Populate a tree by completing the treeFactory function.
...
/*******************************************************************
  Task 2: Implement level-order traversal in the traverseLevels function.
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while (!nodesToExplore.empty()) {
    ...
    // Make a copy of the front pointer in the queue, then pop it to decrease the queue
